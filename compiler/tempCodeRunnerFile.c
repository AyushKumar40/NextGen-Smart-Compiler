...
ASTNode *parseExpression(MiniCompiler *compiler, int minPrec)
{
    ...
    {
        Token token = currentToken(compiler);
        if (token.type != OP && token.type != COMPARE)
            break;
...
ASTNode *parseProgramBlock(MiniCompiler *compiler)
{
    ...
    while (currentToken(compiler).type != RBRACE && currentToken(compiler).type != END)
    {
        ...
        {
        case VAR:
            addChild(block, parseDeclaration(compiler));
            break;
        case PRINT:
            addChild(block, parsePrint(compiler));
            break;
        case IF:
            addChild(block, parseIfElse(compiler));
            break;
        case RETURN:
            addChild(block, parseReturn(compiler));
            break;
        default:
            addError(compiler, "Unexpected token in block");
            advance(compiler);
            break;
        }
    }
    if (currentToken(compiler).type == RBRACE)
        advance(compiler);
    return block;
}
ASTNode *parseProgram(MiniCompiler *compiler)
{
    ...
    while (currentToken(compiler).type != END)
    {
        ...
        case VAR:
        {
            ASTNode *decl = parseDeclaration(compiler);
            if (decl)
                addChild(program, decl);
            break;
        }
        case PRINT:
        {
            ASTNode *prt = parsePrint(compiler);
            if (prt)
                addChild(program, prt);
            break;
        }
        case IF:
        {
            ASTNode *ifstmt = parseIfElse(compiler);
            if (ifstmt)
                addChild(program, ifstmt);
            break;
        }
        case RETURN:
        {
            ASTNode *ret = parseReturn(compiler);
            if (ret)
                addChild(program, ret);
            break;
        }
        default:
            addError(compiler, "Unexpected token in program");
            advance(compiler);
            break;
        }
    }
...
void compile(MiniCompiler *compiler, const char *filename)
{
    ...
    printf("\nParse Tree:\n");
    if (compiler->ast)
        printAST(compiler->ast);

    printf("\nParse Tree (JSON):\n");
    if (compiler->ast)
        printASTJSON(compiler->ast, stdout, 0);
    printf("\n");
...
#!/ bin / bash
    ...
    gcc ayush.c - o ayush &&./ ayush pbl.mini
#!/ bin / bash

                                   gcc ayush.c -
        o ayush &&./ ayush pbl.mini
